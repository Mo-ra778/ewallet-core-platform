    ...
    <div class="max-w-sm w-full space-y-6">
        ...
        <div class="bg-white border border-slate-200/90 rounded-xl p-6 sm:p-8 shadow-sm space-y-5">
            ...
            <form action="{{ route('agent.login') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1.5">رقم هاتف الوكيل</label>
                    <input type="text" name="phone" value="{{ old('phone', '777000111') }}" required autofocus dir="ltr"
                           class="w-full px-3.5 py-2 text-xs bg-slate-50 border border-slate-200 rounded-lg text-slate-900 focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-700 focus:border-transparent transition text-right font-mono">
                </div>
